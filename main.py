...
def get_args():
    ...
    parser.add_argument("--dev_output", type=str, default="cfimdb-dev-output.txt")  # output for dev
    parser.add_argument("--test_output", type=str, default="cfimdb-test-output.txt")  # output for dev
...
def pad_sentences(sents, pad_id):
    ...
    Args:
        sents: list(list(int)), a list of a list of word ids representing sentences
        pad_id: int, the word id of the "<pad>" token
        
    Returns:
        aug_sents: list(list(int)), a list of padded sentences ensuring |s_1| == |s_i|, for s_i in sents
    """
    max_length = max(len(sentence) for sentence in sents)  # Find the maximum sentence length
    aug_sents = [sentence + [pad_id] * (max_length - len(sentence)) for sentence in sents]  # Pad sentences
